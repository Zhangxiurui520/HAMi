...
func TestConvertMarsSDeviceToHAMIDevice(t *testing.T) {
	for _, ts := range []struct {
		name  string
		mars []*MarsSDeviceInfo
		...
		t.Run(ts.name, func(t *testing.T) {
			result := convertMarsSDeviceToHAMIDevice(ts.mars)

			if !reflect.DeepEqual(ts.expected, result) {
				t.Errorf("convertMarsSDeviceToHAMIDevice failed: result %v, expected %v",
					result, ts.expected)
			}
		})
	}
...
func TestConvertHAMIPodDeviceToMarsPodDevice(t *testing.T) {
	for _, ts := range []struct {
		name string
		hami device.PodSingleDevice

		expected PodMarsSDevice
	}{
		{
			name: "base test",
			hami: device.PodSingleDevice{
		...
		t.Run(ts.name, func(t *testing.T) {
			result := convertHAMIPodDeviceToMarsPodDevice(ts.hami)
