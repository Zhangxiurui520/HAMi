...
func TestGetNodeDevices(t *testing.T) {
	MarsResourceCount = "mars-tech.com/gpu"
			...
			node: corev1.Node{
				...
				Status: corev1.NodeStatus{
					Capacity: corev1.ResourceList{
						corev1.ResourceName(MarsResourceCount): *resource.NewQuantity(1, resource.DecimalSI),
					},
				},
			},
			expected: []*device.DeviceInfo{
...
func TestParseMarsAnnos(t *testing.T) {
	tests := []struct {
		name  string
		index int
		value float32
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			value := parseMarsAnnos(tt.name, tt.index)
			if value != tt.value {
				...
				ctr: &corev1.Container{
					Resources: corev1.ResourceRequirements{
						Limits: corev1.ResourceList{
							"mars-tech.com/gpu": resource.MustParse("1"),
						},
					},
				},
				p: &corev1.Pod{},
	...
	tests := []struct {
	...
	}{
		{
			...
			args: struct {
				...
				allocated: &device.PodDevices{
					MarsGPUDevice: device.PodSingleDevice{
				...
				node: &corev1.Node{
					ObjectMeta: metav1.ObjectMeta{
						Annotations: map[string]string{
							"mars-tech.com/gpu.topology.losses": "{\"1\":100,\"2\":200}",
						},
					},
				},
				podDevices: device.PodSingleDevice{
		...
		},
		{
			...
			args: struct {
				...
				node: &corev1.Node{
					ObjectMeta: metav1.ObjectMeta{
						Annotations: map[string]string{
							"mars-tech.com/gpu.topology.scores": "{\"1\":100,\"2\":200}",
						},
					},
				},
				podDevices: device.PodSingleDevice{
...
func TestMarsDevices_Fit(t *testing.T) {
	config := MarsConfig{
		ResourceCountName: "mars-tech.com/gpu",
	}
	dev := InitMarsDevice(config)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.wantFit {
				if len(result[MarsGPUDevice]) != test.wantLen {
					t.Errorf("expected len: %d, got len %d", test.wantLen, len(result[MarsGPUDevice]))
				}
				for idx, id := range test.wantDevIDs {
					if id != result[MarsGPUDevice][idx].UUID {
						t.Errorf("expected device id: %s, got device id %s", id, result[MarsGPUDevice][idx].UUID)
					}
				}
			}
...
func TestMarsDevices_AddResourceUsage(t *testing.T) {
	tests := []struct {
		name        string
